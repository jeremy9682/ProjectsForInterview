...
public class StudentTests {
    ...
    public void testOpenAddressingResizeWhenInsert() {
       ...
       LinearProbingHashTable lp = new LinearProbingHashTable();
        ...
        // But the following ones should trigger a resizing.

        lp.remove("Tiffany");
        assertEquals("Linear Probing hash with starting capacity of 7 should have a capacity of 7 after " +
                "five insertions and a successful deletion.", 7, lp.capacity());

        qp.remove("Tiffany");

        assertEquals("Quadratic Probing hash with starting capacity of 7 should have a capacity of 7 after " +
                "five insertions and a successful deletion.", 7, qp.capacity());
    ...
    public void ownTest() {
        ...
        HashTable qp = new LinearProbingHashTable();
        for(int i= 0; i< 1000;i++){
            String insert = Integer.toString(i);
            qp.put(insert,insert);
            System.out.println("Success put" + insert);
        }
        for(int i= 0; i< 1000;i++){
            String insert = Integer.toString(i);
            if(i == 5){
            ...
            qp.remove(insert);
            System.out.println("Success remove" + insert);
        }

        /*
        int result = (int) 'I' - (int) 'B';
